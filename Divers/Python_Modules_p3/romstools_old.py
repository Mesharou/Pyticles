...
def get_depths(fname,gname,tindex,type):
    ...
    if type=='w':
        try:
            Cs = nc.variables['Cs_w'][:]
        except:
            Cs = nc.Cs_w
        N=N+1
    else:
        try:
            Cs = nc.variables['Cs_r'][:]
        except:
            Cs = nc.Cs_r
...
def get_depths_zoom(fname,gname,tindex,type,ny1,ny2,nx1,nx2, ncname=None):
    ...
    if not np.isreal(Cs[0]):
        grd = Dataset(ncname.grd, 'r', format='NETCDF3_CLASSIC')
        if type=='w':
            try:
                Cs = grd.variables['Cs_w'][:]
            except:
                Cs = grd.Cs_w
        else:
            try:
                Cs = grd.variables['Cs_r'][:]
            except:
                Cs = grd.Cs_r
    #########
    
    if zeta.shape==h.shape:
        #print(h.shape,zeta.shape,hc,N,Cs)
